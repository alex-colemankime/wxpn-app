...
const NowPlaying = ({ open, onClose, playing, setPlaying }) => (
  <div style={{
    ...
    {/* On Air info at bottom */}
    <div style={{
      padding: "16px 20px 44px", flexShrink: 0,
      borderTop: `1px solid ${C.divider}`,
      display: "flex", alignItems: "center", gap: 12,
    }}>
      <img src={SHOWS.middays.img} alt="" style={{ width: 44, height: 44, borderRadius: 10, objectFit: "cover" }} />
      <div style={{ flex: 1 }}>
        <div style={{ fontSize: 12, fontWeight: 700, color: C.textDim, letterSpacing: "0.14em", fontFamily: F.display }}>ON AIR</div>
        <div style={{ fontSize: 16, fontWeight: 600, color: C.cream, fontFamily: F.display }}>{SHOWS.middays.name}</div>
      </div>
      <span style={{ fontSize: 13, color: C.textDim, fontFamily: F.mono }}>{SHOWS.middays.time}</span>
    </div>
  </div>
);
