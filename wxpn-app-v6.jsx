...
/* ═══════════════ LIVE SCREEN ═══════════════ */
const LiveScreen = ({ playing, setPlaying }) => {
  const tracks = [
...
export default function WXPNApp() {
  ...
  const nav = id => { setScreen(id); setSub(null); setSettings(false); };
  ...
  const content = () => {
    ...
    return <LiveScreen playing={playing} setPlaying={setPlaying} />;
  };
  ...
  return (
    <div style={{
      ...
      <Phone>
        ...
        />
        {content()}
        {showMini && <Mini onTap={() => nav("live")} playing={playing} />}
